...
import { Crosshair, FileText, CheckSquare, AlignLeft, Maximize2, Check, Edit } from 'lucide-react';
            ...
            {raidOperations.map(({ raid, step }) => {
                ...
                return (
                    <div 
                        ...
                        className={`bg-life-paper border-l-4 rounded-r-xl shadow-lg relative overflow-hidden group transition-all duration-300 cursor-pointer ${DIFFICULTY_COLORS[raid.difficulty].split(' ')[0]} hover:bg-white/5`}
                    >
                        ...
                        <div className="p-3">
                            <div className="flex justify-between items-start relative z-10">
                                <div className="flex-1 min-w-0 pr-2">
                                    <div className="flex items-center gap-2 mb-1">
                                        <span className="text-[9px] bg-life-black px-1.5 py-0.5 rounded text-life-muted font-bold uppercase tracking-wider border border-life-muted/20">
                                            OP: {raid.title}
                                        </span>
                                    </div>
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <button 
                                        ...
                                        className="w-8 h-8 rounded-lg bg-life-black border border-life-muted/30 flex items-center justify-center text-life-muted hover:text-life-gold hover:border-life-gold transition-all relative z-20"
                                    >
                                        <CheckSquare size={16} />
                                    </button>
                                </div>
                            ...
                            {showNotes && (
                                <div className="mt-3 pt-3 border-t border-life-muted/10 animate-in slide-in-from-top-2 cursor-default" onClick={(e) => e.stopPropagation()}>
                                    ...
                                    <div className="flex justify-end mt-2">
                                        <button onClick={(e) => { e.stopPropagation(); openRaidStepDetails(step.id, raid.id); }} className="text-[9px] flex items-center gap-1 text-life-muted hover:text-life-gold bg-life-black px-2 py-1 rounded border border-life-muted/20">
                            ...
                            {showSubtasks && (
                                <div className="mt-3 pt-3 border-t border-life-muted/10 animate-in slide-in-from-top-2 cursor-default" onClick={(e) => e.stopPropagation()}>
                                    ...
                                    ) : <div className="text-[10px] text-life-muted italic text-center py-2 border border-dashed border-life-muted/10 rounded mb-2">No subtasks defined.</div>}
                                </div>
                            )}
                        </div>
                    </div>
